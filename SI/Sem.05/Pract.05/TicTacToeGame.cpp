...
bool aPlayerHasWon(int player) {
    for(int i = 0; i < 3; i++) {
        if(board[i][0] + board[i][1] + board[i][2] == (3 * player)) return true;
        if(board[0][i] + board[1][i] + board[2][i] == (3 * player)) return true;
    }
    if(board[0][0] + board[1][1] + board[2][2] == (3 * player) || board[0][2] + board[1][1] + board[2][0] == (3 * player)) return true;
    return false;
}
